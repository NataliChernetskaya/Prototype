				...
				String coreName = nam.getText();
				 if(coreName.trim().length()>0){
					...
					while(Edit_cpu.model.getRowCount() > 0){
						Edit_cpu.model.removeRow(0);
					}
					
					String Response1 = target.path("rest").path("getinfo").path("core").path(Edit_cpu.ecuId).request().accept(MediaType.TEXT_PLAIN).get(String.class);
					...
					if(f==1)
					 ...
					 JLabel countLabel = new JLabel("Введите имя ядра!"); 
		             JOptionPane.showMessageDialog(null, countLabel);
				 }
